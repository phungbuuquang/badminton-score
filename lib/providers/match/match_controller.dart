...
class MatchController extends StateNotifier<MatchState> {
  ...
  void decreaseA() {
    if (state.scoreA > 0) state = state.copyWith(scoreA: state.scoreA - 1);
  }
  ...
  void decreaseB() {
    if (state.scoreB > 0) state = state.copyWith(scoreB: state.scoreB - 1);
  }
  ...
  void toggleSwap() => state = state.copyWith(swapped: !state.swapped);
}
...
final matchControllerProvider =
    StateNotifierProvider.family<MatchController, MatchState, (String, String)>(
      (ref, args) {
        ...
        final currentRules = ref
            .read(rulesStreamProvider)
            .maybeWhen(data: (r) => r, orElse: () => SetRules.defaults());
